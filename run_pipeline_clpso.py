...
logger = logging.getLogger("run_pipeline_clpso")


def parse_args():
    parser = argparse.ArgumentParser(description="Multi-Asset Portfolio Optimization Pipeline (CLPSO)")
    ...
    parser.add_argument("--particles", type=int, default=500, help="Number of particles for CLPSO")
...
def main():
    ...
    # 0. Create timestamped output directory: output/clpso_{DD_M_HH_MM}
    now = datetime.now()
    run_tag = now.strftime("clpso_%d_%m_%H_%M")
    ...
    # A. Run walk-forward with CLPSO weight optimization for each static selection strategy
    for name, df in portfolio_dfs.items():
        if df.empty:
    ...
    # B. Run CLPSO walk-forward optimization (Full Universe)
    ...
    test_len = len(clpso_res["OOS_Returns_Array"])
...
def save_markdown_report(all_results, spy_res, results_2025=None, spy_res_25=None, output_path="performance_report.md"):
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        f.write("# Performance Report: Walk-Forward Backtesting Metrics (CLPSO)\n\n")
        f.write("This report summarizes the walk-forward out-of-sample (OOS) simulation performance metrics using CLPSO weight optimization, comparing results with and without transaction costs. Metrics display Mean ± Standard Deviation across trials.\n\n")
