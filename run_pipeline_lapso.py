...
logger = logging.getLogger("run_pipeline_lapso")


def parse_args():
    parser = argparse.ArgumentParser(description="Multi-Asset Portfolio Optimization Pipeline (LAPSO)")
    ...
    parser.add_argument("--particles", type=int, default=500, help="Number of particles for LAPSO")
...
def main():
    ...
    out_dir = f"output/run_{timestamp}"
    ...
    # A. Run walk-forward with LAPSO weight optimization for each static selection strategy
    for name, df in portfolio_dfs.items():
        if df.empty:
    ...
    # B. Run LAPSO walk-forward optimization (Full Universe)
    ...
    test_len = len(lapso_res["OOS_Returns_Array"])
...
def save_markdown_report(all_results, spy_res, results_2025=None, spy_res_25=None, output_path="performance_report.md"):
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        f.write("# Performance Report: Walk-Forward Backtesting Metrics (LAPSO)\n\n")
        f.write("This report summarizes the walk-forward out-of-sample (OOS) simulation performance metrics using LAPSO weight optimization, comparing results with and without transaction costs. Metrics display Mean ± Standard Deviation across trials.\n\n")
