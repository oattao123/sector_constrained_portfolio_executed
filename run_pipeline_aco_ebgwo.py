...
logger = logging.getLogger("run_pipeline_aco_ebgwo")


def parse_args():
    parser = argparse.ArgumentParser(description="Multi-Asset Portfolio Optimization Pipeline (ACO+EBGWO)")
...
def main():
    ...
    # 0. Create timestamped output directory: output/aco_ebgwo_{DD_M_HH_MM}
    now = datetime.now()
    run_tag = now.strftime("aco_ebgwo_%d_%m_%H_%M")
...
def save_markdown_report(all_results, spy_res, results_2025=None, spy_res_25=None, output_path="performance_report.md"):
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        f.write("# Performance Report: Walk-Forward Backtesting Metrics (ACO+EBGWO)\n\n")
        f.write("This report summarizes the walk-forward out-of-sample (OOS) simulation performance metrics using ACO+EBGWO weight optimization, comparing results with and without transaction costs. Metrics display Mean ± Standard Deviation across trials.\n\n")
