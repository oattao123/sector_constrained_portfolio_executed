...
logger = logging.getLogger("run_pipeline_pso")


def parse_args():
    parser = argparse.ArgumentParser(description="Multi-Asset Portfolio Optimization Pipeline (PSO)")
...
def main():
    ...
    # 0. Create timestamped output directory: output/pso_{DD_M_HH_MM}
    now = datetime.now()
    run_tag = now.strftime("pso_%d_%m_%H_%M")
    ...
    # A. Run walk-forward with PSO weight optimization for each static selection strategy
    for name, df in portfolio_dfs.items():
        if df.empty:
    ...
    # B. Run PSO walk-forward optimization (Full Universe)
    ...
    test_len = len(pso_res["OOS_Returns_Array"])
...
def save_markdown_report(all_results, spy_res, results_2025=None, spy_res_25=None, output_path="performance_report.md"):
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        f.write("# Performance Report: Walk-Forward Backtesting Metrics (PSO)\n\n")
        f.write("This report summarizes the walk-forward out-of-sample (OOS) simulation performance metrics using PSO weight optimization, comparing results with and without transaction costs. Metrics display Mean ± Standard Deviation across trials.\n\n")
